...
int CSV_sheet_Init(void){
    ...
    if(strstr(GPS_ZDA_Data.today.year,"20") != NULL)
...
int SDCard_Write_log(GPS_Data GGA_Result_Data, GPS_Data ZDA_Result_Data){
	if(SDCard_states == 0){
		FIL fp;
		FRESULT res;
		uint16_t File_Num = 1;
		...
		if(strstr(ZDA_Result_Data.today.year,"20") != NULL)
			Format_FileName_Date(file_name_buf, ZDA_Result_Data.today);
		else
			sprintf(file_name_buf,"250101");
	...
	//    	printf("f_open fail: %s\r\n", USER_FILE_NAME);
		...
		case FR_OK://文件存在
				char write_bf[50];
					...
					Format_GPS_Date(LogBuf.Date,ZDA_Result_Data.today);
					Format_GPS_Time(LogBuf.Time,ZDA_Result_Data.time);
